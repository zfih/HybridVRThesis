...
void SSAO::Render( GraphicsContext& GfxContext, const float* ProjMat, float NearClipDist, float FarClipDist, DepthBuffer* curDepthBuf, Cam::CameraType CameraType)
{
    ...
    if (!Settings::SSAO_Enable)
    {
        ...
        {
            Context.TransitionResource(g_SceneColorBuffer, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, true);
    ...
    GfxContext.TransitionResource(*curDepthBuf, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    ...
    if (Settings::SSAO_DebugDraw)
    {
        ...
        CC.TransitionResource(g_SceneColorBuffer, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, true);
