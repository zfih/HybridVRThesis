...
D3D12_RESOURCE_BARRIER& CommandContext::GetNextFreeBarrier()
{
	ASSERT(m_NumBarriersToFlush < 16,
	       "Exceeded arbitrary limit on buffered barriers");
	return m_ResourceBarrierBuffer[m_NumBarriersToFlush++];
}
...
void GraphicsContext::ClearUAV(GpuBuffer& Target)
{
	...
	m_CommandList->ClearUnorderedAccessViewUint(GpuVisibleHandle, Target.GetUAV(), Target.GetResource(), ClearColor, 0,
	                                            nullptr);
}
...
void GraphicsContext::ClearUAV(ColorBuffer& Target)
{
	...
	m_CommandList->ClearUnorderedAccessViewFloat(GpuVisibleHandle, Target.GetUAV(), Target.GetResource(), ClearColor, 1,
	                                             &ClearRect);
}
...
void GraphicsContext::ClearDepth(DepthBuffer& Target, int subDSV)
{
	...
	m_CommandList->ClearDepthStencilView(Target.GetSubDSV(subDSV), D3D12_CLEAR_FLAG_DEPTH, Target.GetClearDepth(),
	                                     Target.GetClearStencil(), 0, nullptr);
}
	...
	UINT Subresource)
{
		...
		UINT Subresource)
	{
		...
		{
			D3D12_RESOURCE_BARRIER& barrierDesc = GetNextFreeBarrier();
	...
	else if (Resource.m_ConsiderSubresources && Resource.m_SubresourceCount > 0)
	{
		// Transition all subresources
		for (UINT subresourceIndex = 0;
	...
	{
		// Transition resource and all subresources
		// NOTE: Will result in errors if used on a resource with subresources
		// in different states!
		pushTransition(Resource.m_UsageState, Subresource);
	}
}
...
void CommandContext::InitializeTexture(GpuResource& Dest, UINT NumSubresources, D3D12_SUBRESOURCE_DATA SubData[])
{
	...
	UpdateSubresources(InitContext.m_CommandList, Dest.GetResource(), mem.Buffer.GetResource(), 0, 0, NumSubresources,
	                   SubData);
...
void CommandContext::ReadbackTexture2D(GpuResource& ReadbackBuffer, PixelBuffer& SrcBuffer)
{
	...
	g_Device->GetCopyableFootprints(&SrcBuffer.GetResource()->GetDesc(), 0, 1, 0, &PlacedFootprint, nullptr, nullptr,
	                                nullptr);
