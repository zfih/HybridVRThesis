...
void D3D12RaytracingMiniEngineSample::Startup(void)
{
	...
	g_SceneNormalBuffer.Create(L"Main Normal Buffer", g_SceneColorBuffer.GetWidth(), g_SceneColorBuffer.GetHeight(), 1,
...
{
	std::wstring cameraName = L"Camera: " + std::to_wstring(cam);
	...
	const bool skipShadowMap =
		...
		rayTracingMode == RTM_TRAVERSAL ||
		rayTracingMode == RTM_SSR;
	GraphicsContext& gfxContext = GraphicsContext::Begin(std::wstring(L"Render pass") + cameraName);
...
void D3D12RaytracingMiniEngineSample::Raytrace(class GraphicsContext& gfxContext, UINT cam)
{
	...
	case RTM_SSR:
		RaytracebarycentricsSSR(gfxContext, *m_Camera[cam], g_SceneColorBuffer, g_SceneDepthBuffer,
		                        g_SceneNormalBuffer);
		break;
